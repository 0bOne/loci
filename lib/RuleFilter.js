...
module.exports = class RuleFilter
{
    ...
    _loadFile(fileInfo)
    {
        ...
        const content = buffer.toString(fileInfo.encoding);
        return content;
    }
    ...
    GetStatistics(fileInfo, definition)
    {
        ...
        let content = this._loadFile(fileInfo);
        ...
        fileInfo.timing.analysis += this._stopwatch.ElapsedTime();
    ...
    _removeBlanks(lines)
    {
        ...
        {
            let trimmedLine = line.replaceAll("\t", "").trim();
    ...
    _applyFilter(content, filter, definition)
    {
        ...
        {
            case "remove":
                content = content.replaceAll(filter.expression, "");
                break;
            case "replace":
                content = content.replaceAll(filter.expression, filter.with);
                break;
            case "remove-between":
    ...
    _indentedBlockFilter(content, filter)
    {
        ...
        for (let line of lines)
        {
            let lineToTest = line.replaceAll("\t", "    ");
            if (startingIndent > 0)
            {
    ...
    // _removeWithinAllStrings_First(content, quoteSequence, what)
    // {
